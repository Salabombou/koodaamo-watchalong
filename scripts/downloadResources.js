...
function resolveCloudTorrentAsset(release, target) {
  ...
  const checksumsAsset = (release.assets || []).find(
    (candidate) => candidate.name === "cloud-torrent_0.9.4_checksums.txt" || /checksums\.txt$/.test(candidate.name),
  );
...
async function downloadCloudTorrent(target) {
  ...
  const binaryName = target.platform === "win32" ? "cloud-torrent.exe" : "cloud-torrent";
...
async function main() {
  ...
  if (forceDownload || !hasExistingFfmpegBinaries(target)) {
    ...
    console.log("[resources] ffmpeg/ffprobe already present, skipping download");
  }
